...
source("/zfs/analysis/r_functions/deseq2_function_20190903.R")
source("/zfs/analysis/r_functions/pca_explorer_20190905.R")
...
p_pseudoaligned <- read.csv(file = "metadata/p_pseudoaligned_trap_20190905.txt", sep = "\t", header = T) %>%
  as_tibble()

sample_metadata <- read.csv(file.path(base_dir, 'metadata/sample_metadata_110719.csv'), header = T) %>%
  as_tibble() %>%
  inner_join(p_pseudoaligned, by = c("sample_code" = "sample")) %>%
  mutate(align_rank=rank(p_pseudoaligned)/length(p_pseudoaligned))

sample_metadata_KW_batch_2_pca <- sample_metadata %>% 
  as_tibble() %>%
  filter(ip_pool == 'n' & batch_trap == 'b2')

sample_metadata_PK_batch_1_pca <- sample_metadata %>% 
  as_tibble() %>%
  filter(ip_pool == 'n' & batch_trap == 'b3')
...
g <- ggplot(data = sample_metadata_PK_batch_1_pca, mapping = aes(x = reorder(sample_name, align_rank), fill = outlier))

g + geom_bar(width = 0.75, position = "dodge", aes(weight = p_pseudoaligned)) +
  coord_cartesian(ylim=c(65, 85)) +
  theme_bw() +
  scale_color_lancet() +
  scale_fill_lancet() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1), axis.title.x = element_blank()) +
  ylab("Percent pseudoaligned")

g + geom_bar(width = 0.75, position = "dodge", aes(weight = align_rank)) +
  theme_bw() +
  scale_color_lancet() +
  scale_fill_lancet() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1), axis.title.x = element_blank()) +
  ylab("Pseudoalignment Percentile")

```


**PCA of PK Batch 1 IP samples**
```{r}
name = 'pk_batch_1_ip'
...
run_pcaExplorer()
```
**PCA of PK Batch 1 IP MB samples**
```{r}
