...
Importing Trinity/kallisto counts
```{r}
...
trinity_path <- "/zfs/analysis/PK_TRAP/trinity_kallisto_unbound_mb_axon_kw_pk/"

name = 'axon_ub_vs_mb_ub'
metadata_filter = "ip == 'input' & batch_trap %in% c('b2','b3')"
design <- ~ batch_trap + age + axon

trinity_sample_metadata <- sample_metadata_all %>% data.frame() %>%
                    as_tibble() %>% filter(eval(parse(text=metadata_filter)))

#create list of kallisto input files
trinity_files <- file.path(trinity_path, trinity_sample_metadata$sample_code, "abundance.h5")
names(trinity_files) <- trinity_sample_metadata$sample_name

#import kallisto files
trinity_txi <- tximport(trinity_files, type = "kallisto", tx2gene = trinity_tx2gene, ignoreTxVersion = F, geneIdCol = "GENEID")
  
#create deseq2 object
trinity_dds <- DESeqDataSetFromTximport(trinity_txi, colData = trinity_sample_metadata, design = design)

#calculate XYZ
trinity_dds <- DESeq(trinity_dds, minReplicatesForReplace = replicate_replace)
trinity_counts <- as.data.frame(counts(trinity_dds, normalized = T)) %>%
  rownames_to_column(var = "gene") %>%
  as_tibble()

egfp_l10a_counts <- trinity_counts %>%
  filter(gene == 'eGFP-L10a') %>%
  gather(sample,counts,-gene) %>%
  inner_join(sample_metadata_all, by = c('sample' = 'sample_name')) %>%
  select(gene, sample, region, counts)

ires_cre_counts <- trinity_counts %>%
...
eGFP_multi_boxplot <- ggplot(egfp_l10a_counts, aes(x = region, y = counts, fill = region)) +
  geom_boxplot(outlier.size = 0) +
  geom_point(pch = 21) +
  theme_bw() + 
  labs(fill = 'Region') +
  ...
  scale_color_npg() +
  scale_fill_npg() +
  ggtitle('eGFP-L10a Transcript Counts in Input RNA')

eGFP_multi_boxplot

cre_multi_boxplot <- ggplot(ires_cre_counts, aes(x = region, y = counts, fill = region)) +
  geom_boxplot(outlier.size = 0) +
  geom_point(pch = 21) +
  theme_bw() + 
  labs(fill = 'Region') +
