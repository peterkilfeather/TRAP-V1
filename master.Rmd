...
```{r}
p_pseudoaligned <- read.csv(file.path(base_dir, 'metadata/p_pseudoaligned_trap_20190905.txt'), sep = '\t', header = T) %>%
  as_tibble()
...
th_txdb <- AnnotationDbi::select(txdb, keys = keys, columns=columns(txdb), keytype="GENEID")
GR <- transcripts(txdb)
show(GR[1:3])
strand(GR[1:3])
sum(runLength(strand(GR[1:3])))
length(GR[1:3])
...
transcripts(txdb, filter=TxidFilter("ENST00000215832"))


available.genomes()
Mmusculus <- getBSgenome("BSgenome.Mmusculus.UCSC.mm10", masked = FALSE)
tx_seqs_th <- extractTranscriptSeqs(Mmusculus, TxDb.Hsapiens.UCSC.hg19.knownGene,use.names=TRUE)
...
```
Striatum IP Outlier Evidence
```{r fig.height = 4, fig.width = 8}

name = 'pk_batch_1_ip_striatum'
metadata_filter = "batch_trap %in% c('b3') & ip == 'ip' & region != 'mb'"
batch_correct = F
batch = 'region'
skip_pcaExplorer = F
